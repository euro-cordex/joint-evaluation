...
special_case = ["WRF451Q", "RegCM5-0", "HadREM3-GA7-05"]
...
def regrid(ds, regridder, mask_after_regrid=False):
...
def fix_360_longitudes(dataset: xr.Dataset, lonname: str = "lon", reindex = False) -> xr.Dataset:
    """
    ...
    if lon.max().values > 180 and lon.min().values >= 0:
        ...
        if reindex:
            ...
            lon_1d = np.ravel(dataset[lonname].values)  # aplana cualquier dimensión extra
            lon_sorted = np.sort(lon_1d)
            dataset = dataset.reindex({lonname: lon_sorted})
    return dataset
...
def plot_regional_biases(**kwargs):
    ...
    for i, region in enumerate(regs):
        ...
        for j, season in enumerate(seasons):
            ...
            cmip6_median_fp = np.nanmean(abs(select_by_region_season(df_CMIP6_fp_fair, region, season)))
            cmip5_median_fp = np.nanmean(abs(select_by_region_season(df_CMIP5_fp_fair, region, season)))
            cmip6_median_sp = np.nanmean(abs(select_by_region_season(df_CMIP6_sp_fair, region, season)))
            cmip5_median_sp = np.nanmean(abs(select_by_region_season(df_CMIP5_sp_fair, region, season)))
